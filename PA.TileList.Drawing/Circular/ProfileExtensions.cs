...
namespace PA.TileList.Drawing.Circular
{
    public static class ProfileExtentions
    {
        public static RectangleD<Bitmap> GetImage(this CircularProfile p, int width, int height, RectangleF inner,
            ScaleMode mode = ScaleMode.NONE, Pen radiusPen = null, Pen arcPen = null, Pen extraPen = null)
        {
            return p.GetImage(new RectangleD<Bitmap>(new Bitmap(width, height), inner, mode), radiusPen, arcPen,
                extraPen);
        }
            ...
            ScaleMode mode = ScaleMode.NONE, Pen radiusPen = null, Pen arcPen = null, Pen extraPen = null)
        {
            return
                p.GetImage(
                    new RectangleD<Bitmap>(new Bitmap(width, height), new PointF(-width/2f, -height/2f),
                        new SizeF(width, height), mode), radiusPen, arcPen, extraPen);
        }
        ...
        public static RectangleD<U> GetImage<U>(this CircularProfile p, RectangleD<U> image, Pen radiusPen = null,
            Pen arcPen = null, Pen extraPen = null)
            where U : Image
        {
            using (var g = image.GetGraphicsD(extraPen))
            {
                var maxsize = (float) p.GetMaxRadius()*2f;
                var minsize = (float) p.GetMinRadius()*2f;
                var midsize = (float) p.Radius*2f;
                ...
                foreach (var current in p.Profile)
                {
        ...
        public static void DrawStep(this GraphicsD g, CircularProfile.ProfileStep last,
            CircularProfile.ProfileStep current, Pen radiusPen = null)
        {
            ...
            if (last.Radius > 0f)
            {
                ...
                g.Graphics.DrawArc(radiusPen ?? Pens.Green, (float) x, (float) y, (float) w, (float) h, (float) ad,
                    (float) sw);
            }
